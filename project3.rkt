...
;; Run the world with a designated spacing, game, controller one
;;and controller two
;;
...
;;a heuristic function will take in a game to be evaluated, and will return
;;a number indicating which player is ahead, and by how much
;;black = bigger number means bigger the advantage
;;white = smaller number means bigger the advantage (more negative)


(: did-I-win? : Heuristic)
;;return a valuation of the current game state... if the black player won,
;;return 1. if the white player won return -1. if the game is ongoing
;;return 0
;;
...
(check-expect
 (did-I-win?
  (Game
   (Board
    (list
     (Stack 4 '(black black black black))
     (Stack 0 '())
     (Stack 0 '())
     (Stack 1 '(white))
     (Stack 1 '(white))
...
(: movement-counter-three-in-a-row
   : Game Pos Integer Integer Integer Player -> Integer)
;;takes in the elements to move in a particular direction designated by string
;;and returns the total number of elements in a line in that given direction
;;dx and dy are taken from the Direction that is inputted in line-of-three?
;;NOTE: POSITION WILL ALWAYS BE THE TOP OF THE GIVEN STACK, STACK-HEIGHT - 1
;;
(define (movement-counter-three-in-a-row gm position dx dy counter plyr-color)
  (cond [(= counter 2) counter]
        [(or
          (= -1 (+ dy (Pos-row position)))
          (= -1 (+ dx (Pos-col position)))
          (= 6 (+ dy (Pos-row position)))
 ...
 (movement-counter-three-in-a-row
  (Game
   (Board
    (list
     (Stack 5 (list 'white 'white 'white 'black 'black))
     (Stack 4 (list 'black 'black 'white 'white))
     (Stack 2 (list 'white 'white))
     (Stack 2 (list 'black 'black))
     (Stack 2 (list 'black 'black))
     (Stack 0 '())
...
;;positive-diagonal, black
(check-expect (movement-counter-three-in-a-row
               (Game
                (Board
                 (list
                  (Stack 5 (list 'white 'white 'white 'black 'black))
...
(: line-of-three? : Game Pos Direction Player -> Boolean)
;;checks to see if there is a line of four in any given direction
;;
(define (line-of-three? gm position dir plyr)
  (and (<= 2
           (+
            (movement-counter-three-in-a-row
             gm
...
(check-expect
 (line-of-three?
  (Game
   ...
   'black)
  (Pos 1 2)
  (Direction "diagonal-negative" 1 -1)
  'white) #t)
...
(check-expect
 (line-of-three?
  (Game
   (Board
    (list
     (Stack 5 (list 'white 'white 'black 'black 'black))
     (Stack 5 (list 'white 'black 'white 'black 'black))
     (Stack 5 (list 'white 'black 'black 'white 'black))
     (Stack 0 '())
     (Stack 4 (list 'black 'black 'black 'white))
     (Stack 4 (list 'black 'black 'black 'white))
     (Stack 4 (list 'black 'black 'black 'white))))
...
(: make-direction-dx-dy : Integer Integer -> Direction)
;;takes in dx and dy integers and returns a direction
;;
(define (make-direction-dx-dy dx dy)
  (match* (dx dy)
    [(1 -1) (Direction "diagonal-negative" 1 -1)]
    [(1 1) (Direction "diagonal-positive" 1 1)]
    [(1 0) (Direction "horizontal" 1 0)]
    [(0 -1) (Direction "vertical" 0 -1)]
    [(_ _) (Direction "N/A" 0 0)]))

(check-expect (make-direction-dx-dy 0 -1) (Direction "vertical" 0 -1))
(check-expect (make-direction-dx-dy 1 0) (Direction "horizontal" 1 0))
(check-expect (make-direction-dx-dy 1 1) (Direction "diagonal-positive" 1 1))
(check-expect (make-direction-dx-dy 1 -1) (Direction "diagonal-negative" 1 -1))
(check-expect (make-direction-dx-dy 0 0) (Direction "N/A" 0 0))
(check-expect (make-direction-dx-dy -1 -1) (Direction "N/A" 0 0))
...
(: within-column : Integer Integer Integer Game Player -> Integer)
;;takes in a row and column value for the position, a counter to
;;keep track of the number of empty spaces there are that can be played
;;to result in a win, a game, and the player who's color we are looking for
;;outputs the number of potential ways to win
;;
(define (within-column r c counter gm plyr)
  (cond [(and
          (< r 6)
          (= 0 (board-ref-player? (Game-board gm) (Pos r c)))
          (line-of-three? gm (Pos (- r 1) c) (make-direction-dx-dy 0 -1) plyr))
         (within-column (+ 1 r) c (+ 1 counter) gm plyr)]
        [(and
          (< r 6)
          (= 0 (board-ref-player? (Game-board gm) (Pos r c)))
          (line-of-three? gm (Pos r (+ 1 c)) (make-direction-dx-dy 1 0) plyr))
         (within-column (+ 1 r) c (+ 1 counter) gm plyr)]
        [(and
          (< r 6)
          (= 0 (board-ref-player? (Game-board gm) (Pos r c)))
          (line-of-three? gm (Pos (+ r 1) (+ c 1))
                          (make-direction-dx-dy 1 1) plyr))
         (within-column (+ 1 r) c (+ 1 counter) gm plyr)]
        [(and
          (< r 6)
          (= 0 (board-ref-player? (Game-board gm) (Pos r c)))
          (line-of-three? gm (Pos (- r 1) (+ c 1)) (make-direction-dx-dy 1 -1)
                          plyr))
         (within-column (+ 1 r) c (+ 1 counter) gm plyr)]
        [(and
          (< r 6)
        ...
        [(and
          (< r 6)
          (= 0 (board-ref-player? (Game-board gm) (Pos r c)))
          (line-of-three? gm (Pos r (- c 1)) (make-direction-dx-dy 1 0) plyr))
         (within-column (+ 1 r) c (+ 1 counter) gm plyr)]
        [(and
          (< r 6)
          (= 0 (board-ref-player? (Game-board gm) (Pos r c)))
        ...
        [(and
         ...
         (within-column (+ 1 r) c (+ 1 counter) gm plyr)]
        [(= 6 r) counter]
        [else (within-column (+ 1 r) c counter gm plyr)]))
...
(: across-row : Integer Integer Integer Game Player -> Integer)
;;counts the number of possible spaces to play to win the game for either
;;player across the entire board
;;
(define (across-row r c counter gm plyr)
  (cond [(= c 7)
         counter]
        [else
         (across-row
          r
          (+ 1 c)
          (+ counter (within-column r c 0 gm plyr))
          gm
          plyr)]))
...
;;calculates the total number of winning spaces for black and white players
;;then returns the difference
;;
(define (count-winning-positions gm)
  (match (outcome gm)
    [(Winning-Line 'black _ _) 1000]
    [(Winning-Line 'white _ _) -1000]
    ['tie 0]
    ['ongoing
     (- (across-row 0 0 0 gm 'black)
        (across-row 0 0 0 gm 'white))]))
  ...
  (Game
   (Board
    (list
     ...
     (Stack 0 '())
     (Stack 1 (list 'black))
     (Stack 6 (list 'white 'black 'white 'white 'white 'black))
     (Stack 5 (list 'white 'black 'black 'white 'black))
     (Stack 4 (list 'white 'black 'white 'white))
     (Stack 0 '())))
   'black)) -1)
...
(check-expect
 (count-winning-positions
  (Game
   (Board
    (list
     (Stack 6 (list 'white 'white 'black 'black 'white 'white))
     (Stack 6 (list 'black 'black 'white 'white 'black 'black))
     (Stack 6 (list 'white 'white 'black 'black 'white 'white))
     (Stack 6 (list 'black 'black 'white 'white 'black 'black))
     (Stack 6 (list 'white 'white 'black 'black 'white 'white))
     (Stack 6 (list 'black 'black 'white 'white 'black 'black))
...
;;------------------------------MINIMAX-EVAL------------------------------
(define-struct Minimax
  ([game : Game]
   [column : Integer]))

(: tie-game : Game)
;;starts a game where there are no more moves and no winning lines on the board
;;the game is tied between the black and the white players
;;
(define tie-game
  (Game
   (Board
    (list
     (Stack 6 '(black black white white black black))
     (Stack 6 '(white white black black white white))
     (Stack 6 '(black black white white black black))
     (Stack 6 '(white white black black white white))
     (Stack 6 '(black black white white black black))
     (Stack 6 '(white white black black white white))
...
(: winning-line-game : Game)
;;starts a game where there are is a single winning line already on the board
;;
(define winning-line-game
  (Game
   (Board
    (list
     (Stack 4 '(white black black white))
     (Stack 3 '(white black black))
     (Stack 2 '(white black))
     (Stack 1 '(white))
     (Stack 0 '())
     (Stack 0 '())
...
(: test-game : Game)
;;the game given from phase three "winning-positions-counter"
;;explanation
;;
(define test-game
  (Game
   (Board
    (list
     ...
     (Stack 0 '())
     (Stack 1 '(black))
     (Stack 6 '(white black white white white black))
     (Stack 5 '(white black black white black))
     (Stack 4 '(white black white white))
     (Stack 0 '())))
   'black))
...
(: game-to-list : Game Integer -> (Listof Minimax))
;;takes in a game and an incrimentor for the column number and outputs a list
;;with the different moves that could be made (max of 7 various games)
;;c should always be 0
;;will never be called on a tied game or a game where one player has already won
;;
(define (game-to-list gm c)
  (cond [(and
          (<= c 6)
          (valid-move? gm (Game-next gm) c))
         (cons (Minimax (apply-move gm (Game-next gm) c) c)
               (game-to-list gm (+ 1 c)))]
        [(> c 6) '()]
        [else (game-to-list gm (+ 1 c))]))
...
(check-expect
 (game-to-list
  (Game
   (Board
    (list
     (Stack 6 '(black white white black black white))
     (Stack 0 '())
     (Stack 0 '())
     (Stack 0 '())
     (Stack 0 '())
   ...
   (Game
    (Board
     (list
      ...
      (Stack 1 '(black))
      (Stack 6 '(white black white white white black))
      (Stack 6 '(black white black black white black))
      (Stack 4 '(white black white white))
      (Stack 0 '())))
    'white)
   4)
   ...
   (Game
    (Board
     (list
      ...
      (Stack 6 '(white black white white white black))
      (Stack 5 '(white black black white black))
      (Stack 5 '(black white black white white))
      (Stack 0 '())))
    'white)
   5)
  (Minimax
...
(: min : (Listof Minimax) Heuristic -> Minimax)
;;returns the smallest number in the inputted list of integers
;;there should and would never be an empty list inputted into the parameters
;;therefor the wild-card case will never be achieved
;;if there is a tie bewtween two scores
;;it will return the last occurance of a game with the tied score
;;
(define (min lsts h)
  (match lsts
    ((cons lst '()) lst)
    ((cons lst lstr)
     (cond [(< (h (Minimax-game lst)) (h (Minimax-game (min lstr h))))
            lst]
           [else (min lstr h)]))
    (_ (error "not an acceptable list"))))




(check-error (min '() count-winning-positions) "not an acceptable list")

(check-expect
 (min (game-to-list test-game 0)
      count-winning-positions)
 (Minimax
  (Game
   (Board
...
(: max : (Listof Minimax) Heuristic -> Minimax)
;;returns the largest number in the inputted list of integers
;;there should and would never be an empty list inputted into hte parameters
;;therefor the wild-card case will never be achieved
;;
(define (max lsts h)
  (match lsts
    ((cons lst '()) lst)
    ((cons lst lstr)
     (cond [(> (h (Minimax-game lst)) (h (Minimax-game (max lstr h))))
            lst]
           [else (max lstr h)]))
    (_ (error "not an acceptable list"))))


(check-error (max '() count-winning-positions) "not an acceptable list")


(check-expect
 (max
  (game-to-list test-game 0)
  count-winning-positions)
 (Minimax
  (Game
   (Board
...
;;takes in a heuristic, a ply (number of levels to check) and a game
;;that acts as the root, and outputs a score assigned to that game
;;using the heuristic function
;;
(define (minimax-eval h ply gm)
  (match (outcome gm)
    ['ongoing
     (cond [(= 0 ply) (h gm)]
           [(symbol=? (Game-next gm) 'black)
            (minimax-eval h (- ply 1)
                          (Minimax-game (max (game-to-list gm 0) h)))]
           [else (minimax-eval h (- ply 1)
                               (Minimax-game (min (game-to-list gm 0) h)))])]
    ['tie (h gm)]
    [(Winning-Line _ _ _) (h gm)]))

(check-expect
 (minimax-eval count-winning-positions 3 new-game)
 0)


;;note- this is not the same board as test-game
;;black won
(check-expect
 (minimax-eval count-winning-positions 3
               (Game
                (Board
                 (list
                  (Stack 1 '(black))
...
;;------------------------------MINIMAX-STRATEGY------------------------------
(: minimax-eval-column : Heuristic Integer Minimax -> Integer)
;;takes in a heuristic, a ply (number of levels to check) and a game
;;that acts as the root, and outputs the column that the bot should play in
;;based on the heuristic inputted
;;
(define (minimax-eval-column h ply mm)
  (match (outcome (Minimax-game mm))
    ['ongoing
     (cond
       [(= 0 ply) (Minimax-column mm)]
       [(symbol=? (Game-next (Minimax-game mm)) 'black)
        (minimax-eval-column h (- ply 1)
                             (max (game-to-list
                                   (Minimax-game mm) 0)
                                  h))]
       [else (minimax-eval-column h (- ply 1)
                                  (min (game-to-list
                                        (Minimax-game mm) 0)
                                       h))])]
    ['tie (Minimax-column mm)]
    [(Winning-Line _ _ _) (Minimax-column mm)]))
 ...
 (minimax-eval-column
  ...
  (Minimax
   (Game
    (Board
     (list
      (Stack 4 '(black black black white))
      (Stack 2 '(white white))
      (Stack 2 '(white black))
      (Stack 1 '(white))
      (Stack 0 '())
...
(define (make-minimax-strategy h ply)
  (lambda ([game : Game])
    (match (outcome game)
      ((Winning-Line plyr b e)
       (error "game already has winner"))
      ('tie
       (error "game already tied"))
      (_ (minimax-eval-column h ply (Minimax game 0))))))



(check-expect
 ((make-minimax-strategy
  count-winning-positions
  3) new-game)
 6)
...
;;note: from phase two, my "run" function is actually named play

#|(play 80
      new-game
      (Bot "Inspector" (make-minimax-strategy count-winning-positions 3))
      (Bot "Blorgon" (make-minimax-strategy count-winning-positions 3)))|#

(play
 80
 new-game
 (Human "Inspector")
     (Bot "Blorgon" (make-minimax-strategy count-winning-positions 2)))
